...
class equilibrium(object):
    ...
    def load_JET(self,shot,time,sequence=0,with_bfield=True,verbose=False):
        ...
        self.rbnd = self._rbnd_data[tind, 0:self._nbnd[tind]-1]
        self.zbnd = self._zbnd_data[tind, 0:self._nbnd[tind]-1]
        ...
        self.xpoints = {"xpm":Point(self._rxpm.data[tind],self._zxpm.data[tind]),
                        "xpl":Point(self._rxpl.data[tind],self._zxpl.data[tind]),
                        "xpu":Point(self._rxpu.data[tind],self._zxpu.data[tind])}


        self.spoints = {"sim":Point(self._rsim.data[tind],self._zsim.data[tind]),
                        "som":Point(self._rsom.data[tind],self._zsom.data[tind]),
                        "sil":Point(self._rsil.data[tind],self._zsil.data[tind]),
                        "sol":Point(self._rsol.data[tind],self._zsol.data[tind]),
                        "siu":Point(self._rsiu.data[tind],self._zsiu.data[tind]),
                        "sou":Point(self._rsou.data[tind],self._zsou.data[tind])}
        ...
        if self.psi is not None:
            ...
            if self.xpoints:
                for xpoint in self.xpoints:
                    ...
                    axes.contour(self.R,self.Z,psi,[psi(self.xpoints[xpoint].r,self.xpoints[xpoint].z)],colors='r',linewidth=2.5,linestyles='solid')
    ...
    def get_fluxsurface(self,psiN,Rref=1.5,Zref=0.0):
        ...
        if type(psiN) is list:
            ...
            for psiNval in psiN:
                ...
                paths = psi_cont.collections[0].get_paths()
